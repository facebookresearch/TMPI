...
def inverse(matrices):
    ...
    if (torch.isnan(inverse)).any():
        print("Matrix inverse contains nan!")
        #raise Exception("Matrix inverse contains nan!")
        inverse = torch.tensor(np.linalg.inv(matrices.cpu().numpy())).to(matrices.get_device())
        # exit()
...
class HomographySample:
    ...
    def sample(self, src_BCHW, d_src_B,
        ...
        R_tnd = R_tgt_src - torch.matmul(t_tgt_src.unsqueeze(2), n.unsqueeze(1)) / -(d_src_B33 + 1e-10)
        ...
        tgt_BCHW = torch.nn.functional.grid_sample(src_BCHW, grid=meshgrid_src, padding_mode='zeros', # was 'border' 
                                                   align_corners=False)
        # BxCxHxW, BxHxW
